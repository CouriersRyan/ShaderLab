Shader "examples/week 12/normals and lighting for ray marching"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 get_normal(float3 surfPos)
            {
                float distAtPos = get_dist(surfPos);
                float sampleDelta = 0.01;
                float2 e = float2(sampleDelta, 0);
                float3 sampleVec = float3(
                    get_dist(surfPos + e.rgg),
                    get_dist(surfPos + e.grg),
                    get_dist(surfPos + e.ggr)
                );
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                float3 indirectSpecular = UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, viewReflection, 0);

                // constrain lighting values to distances that are less than max (only where we hit something)
                float mask = 1-step(MAX_DIST, d);
                diffuse *= mask;

                float directness = 1 - dot(normal, -rayDir);

                // discard pixels
                //if(d >= MAX_DIST) discard;

                color = indirectSpecular * 0.5 + diffuse;
